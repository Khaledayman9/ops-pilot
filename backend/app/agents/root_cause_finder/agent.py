...
class RootCauseFinderAgent:
    ...
    async def run(self, inp: RootCauseFinderInput) -> RootCauseFinderOutput:
        logger.info(f"[RootCauseAgent] RCA for {inp.service} ({inp.severity})")
        ...
        logger.info(
            f"[RootCauseAgent] cause={result.primary_cause[:60]} conf={result.confidence_score}"
        )
        return result
