...
import {
  Zap, ArrowRight, Network, GitBranch, Activity,
  Shield, Layers, Clock, Terminal, AlertTriangle,
} from "lucide-react";
...
type AnimeStatic = {
  ...
  timeline(params?: Record<string, unknown>): { add: (p: Record<string, unknown>, offset?: string | number) => AnimeInstance & { add: (...a: unknown[]) => unknown } };
};
...
const GRAPH_EDGES = [
  ["api","checkout"], ["checkout","payment"], ["checkout","redis"],
  ["checkout","inventory"], ["payment","postgres"], ["inventory","postgres"],
];
const STATUS_COLOR: Record<string, string> = {
  healthy: "#00ff88", degraded: "#ffaa00", critical: "#ff4444",
};
...
const STATS = [
  ...
  { label: "Agent Steps",          value: "5",      sub: "classifier → remediation" },
  { label: "Graph Nodes Traversed",value: "∞",      sub: "bounded by blast radius"  },
];
...
const FEATURES = [
  ...
  { icon: Shield,   title: "Structured LLM Output",  desc: "Every LLM call uses with_structured_output. No free-text parsing, no drift.", accent: "#ff4444" },
  ...
  { icon: Layers,   title: "5-Agent Orchestration",  desc: "LangGraph pipeline: Classifier → Searcher → Graph → RCA → Remediation.", accent: "#00ccff" },
];
...
const SIM_LINES = [
  ...
  { t: 1300, txt: "> [searcher]     entities=6 deployments=['v2.3.1']", color: "#e0e0e0" },
  { t: 2100, txt: "> [graph_agent]  blast_radius=4 upstream=['api-gateway']", color: "#ffaa00" },
  { t: 2900, txt: "> [root_cause]   cause='memory leak in v2.3.1' conf=0.91", color: "#00ccff" },
  { t: 3700, txt: "> [remediation]  rollback=v2.3.0 runbook=RB-001", color: "#e0e0e0" },
...
function HeroGraph() {
  ...
  useEffect(() => {
    loadAnime().then(() => {
      ...
      anime({
        ...
        scale: [0.4, 1],
        duration: 600,
        delay: (el: Element, i: number) => i * 120,
        easing: "easeOutBack",
      });
      ...
      anime({
        ...
        duration: 800,
        delay: (el: Element, i: number) => 400 + i * 80,
        easing: "easeInOutQuad",
      });

      // Critical node pulses
      anime({
        targets: ".hero-pulse",
        r: [6, 14],
        opacity: [0.5, 0],
      ...
      anime({
        ...
        duration: 1800,
        loop: true,
        delay: (el: Element, i: number) => i * 600,
        easing: "linear",
      });
    });
  }, []);
  ...
  return (
    <svg ref={svgRef} viewBox="0 0 640 260" className="w-full h-full">
      ...
      {GRAPH_EDGES.slice(0, 3).map(([a, b], i) => {
        const na = GRAPH_NODES.find((n) => n.id === a)!;
        return (
          <circle
            key={`pk-${i}`}
            className="hero-packet"
            cx={na.x + 55} cy={na.y + 18}
            r={3}
          ...
          <g key={node.id} className="hero-node" style={{ opacity: 0 }}>
            {node.status === "critical" && (
              <circle
                ...
                cx={node.x + 55} cy={node.y + 18}
                r={6} fill={c} opacity={0.5}
              />
            )}
            <rect
              x={node.x} y={node.y} width={110} height={36} rx={6}
              fill="#111118" stroke={c} strokeWidth={node.status === "critical" ? 2 : 1}
  ...
  return (
    <div className="min-h-screen bg-void grid-bg overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-border-1 bg-void/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6 text-xs font-mono">
            <a href="#how-it-works" className="text-chrome-dim hover:text-plasma transition-colors hidden sm:block">How it works</a>
            <a href="#features" className="text-chrome-dim hover:text-plasma transition-colors hidden sm:block">Features</a>
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-plasma animate-pulse" />
              <span className="text-plasma text-xs">ONLINE</span>
            </div>
            <Link href="/chat" className="px-4 py-1.5 bg-plasma text-void font-display font-semibold rounded text-xs hover:bg-plasma-dim transition-colors">
              LAUNCH →
            </Link>
          </div>
        </div>
      ...
      <section className="relative min-h-screen flex items-center pt-14">
        {/* Particle field */}
        <div ref={heroParticlesRef} className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-24 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            >
              Five specialized AI agents traverse your Neo4j service graph, correlate
              deployments, compute blast radius, and stream a full remediation plan
              — in under 5 seconds.
            </motion.p>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            ...
            <p className="text-chrome-dim max-w-xl mx-auto font-mono text-sm">
              ...
              validated Pydantic model — no free-text, no hallucination drift.
            </p>
          </motion.div>

          <div className="flex flex-col md:flex-row gap-2">
            {[
              { n: "01", title: "Classify",  icon: "⚡", color: "#00ff88", desc: "Extracts service, severity P0–P3, incident type, and trigger event." },
              { n: "02", title: "Extract",   icon: "🔍", color: "#00ccff", desc: "Pulls services, deployment IDs, metrics, and error codes from the query." },
              { n: "03", title: "Traverse",  icon: "🕸",  color: "#ffaa00", desc: "Queries Neo4j for dependency graph, blast radius, recent deployments." },
              { n: "04", title: "RCA",       icon: "🧠", color: "#ff4444", desc: "Builds a causal chain with confidence scores and deployment correlation." },
              { n: "05", title: "Remediate", icon: "🔧", color: "#00ff88", desc: "Outputs rollback steps, escalation paths, and runbook references." },
            ].map((step, i) => (
              <motion.div
                key={step.n}
                initial={{ opacity: 0, y: 24 }}
                ...
                className="flex-1 bg-surface-1 border border-border-1 rounded-xl p-5 relative group hover:border-border-2 transition-colors"
              >
                {i < 4 && (
                  <div className="hidden md:flex absolute -right-3 top-1/2 -translate-y-1/2 z-10 text-border-2">
                    →
                  </div>
                )}
                <div className="text-2xl mb-3">{step.icon}</div>
                <div className="text-xs text-chrome-dim font-mono mb-1">{step.n}</div>
                <div className="font-display font-bold text-base mb-2" style={{ color: step.color }}>
                  {step.title}
                </div>
      ...
      <section className="py-32 border-t border-border-1">
        <div className="max-w-3xl mx-auto px-6 text-center">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="font-display font-bold text-chrome mb-6" style={{ fontSize: "clamp(2rem, 4vw, 3.5rem)" }}>
              Ready to{" "}
            ...
            <p className="text-chrome-dim font-mono text-sm mb-10 max-w-xl mx-auto">
              Paste an incident description. Watch five AI agents work in real-time.
      ...
      <footer className="border-t border-border-1 py-8">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between text-xs text-chrome-dim font-mono">
          ...
          <div className="flex items-center gap-4">
            ...
            <a href="https://github.com" className="hover:text-plasma transition-colors">GitHub</a>
          </div>
        </div>
      </footer>
    </div>
