...
function loadTerraformConfig(): TerraformConfig {
  if (typeof window === "undefined") return defaultTerraform();
  try {
    const raw = localStorage.getItem("ops_pilot_terraform_config");
    return raw
      ? { ...defaultTerraform(), ...JSON.parse(raw) }
      : defaultTerraform();
  } catch {
    return defaultTerraform();
  }
}
...
function FieldLabel({
  ...
  return (
    <label className="block text-xs font-mono text-chrome-dim mb-1.5">
      ...
      {optional && (
        <span className="text-chrome-dim ml-1 opacity-50">(optional)</span>
      )}
    </label>
  );
}
...
export default function SettingsPage() {
  ...
  const [terraform, setTerraform] =
    useState<TerraformConfig>(defaultTerraform());
  ...
  function updateGitHub<K extends keyof GitHubConfig>(
    key: K,
  ...
  function updateTerraform<K extends keyof TerraformConfig>(
    key: K,
    value: TerraformConfig[K],
  ) {
    setTerraform((prev) => ({ ...prev, [key]: value }));
  }
  ...
  async function saveGitHubConfig() {
    ...
    try {
      ...
      await fetch(
        `${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000"}/api/v1/settings/github`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function saveTerraformConfig() {
    setTerraformStatus("saving");
    try {
      localStorage.setItem(
        "ops_pilot_terraform_config",
        JSON.stringify(terraform),
      );
      await fetch(
        `${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000"}/api/v1/settings/terraform`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      setTerraformStatus("saved");
      setTimeout(() => setTerraformStatus("idle"), 3000);
    } catch {
  ...
  return (
    <div className="min-h-screen bg-void">
      ...
      <div className="max-w-4xl mx-auto px-6 pt-24 pb-16">
        ...
        >
          <h1 className="font-display text-2xl font-bold text-chrome mb-1">
            Settings
          </h1>
          <p className="text-chrome-dim text-xs font-mono mb-10">
            Configure your LLM provider and GitHub integration. Settings are
            saved locally and synced to the backend.
          </p>
          ...
          <div className="space-y-8">
            ...
            <section className="bg-surface-1 border border-border-1 rounded-xl p-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-display font-semibold text-chrome text-sm">
                      LLM Provider
                    </h2>
                    <p className="text-chrome-dim text-xs font-mono mt-0.5">
                      Configure the AI model used by all agents
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex gap-2">
                    {(["openai", "anthropic", "google"] as const).map((p) => (
                      <button
                        ...
                        className={`flex-1 px-3 py-2 rounded border text-xs font-mono transition-colors ${
                          llm.provider === p
                ...
                <div>
                  <FieldLabel
                    label={`Temperature — ${llm.temperature}`}
                    optional
                  />
                  <input
                    type="range"
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(v) =>
                      updateLLM("max_retries", parseInt(v, 10) || 3)
                    }
                    placeholder="3"
                  />
                </div>
            ...
            <section className="bg-surface-1 border border-border-1 rounded-xl p-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-chrome-dim text-xs font-mono mt-0.5">
                      Enable the repo-scouter agent to correlate incidents with
                      commits and PRs
                    </p>
                  </div>
                </div>
              ...
              <div className="mt-4 p-3 bg-surface-2 border border-border-1 rounded text-xs font-mono text-chrome-dim">
                ...
                <p>
                  The backend uses <code>mcp-server-github</code> via the MCP
                  stdio transport. The token set here is forwarded as{" "}
                  <code>GITHUB_TOKEN</code> to the MCP server process. The
                  repo-scouter agent uses it to fetch recent commits, PRs, and
                  releases for deployment correlation.
                </p>
              </div>
            </section>
            ...
            <section className="bg-surface-1 border border-border-1 rounded-xl p-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <Wrench size={16} className="text-plasma" />
                  <div>
                    <h2 className="font-display font-semibold text-chrome text-sm">
                      Terraform Integration
                    </h2>
                    <p className="text-chrome-dim text-xs font-mono mt-0.5">
                      Enable Terraform Scanner to inspect workspaces, state,
                      drift, and IaC changes
                    </p>
                  </div>
                </div>
                <SaveButton
                  status={terraformStatus}
                  onClick={saveTerraformConfig}
                />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <FieldLabel label="Terraform Cloud Token" optional />
                  <SecretInput
                    value={terraform.terraform_token}
                    onChange={(v) => updateTerraform("terraform_token", v)}
                    placeholder="Terraform Cloud token"
                  />
                  <p className="text-chrome-dim text-xs font-mono mt-1 opacity-60">
                    Used as TERRAFORM_CLOUD_TOKEN and TF_TOKEN_app_terraform_io.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <Input
                    value={terraform.terraform_workspace}
                    onChange={(v) => updateTerraform("terraform_workspace", v)}
                    placeholder="prod-us"
                  />
                  <p className="text-chrome-dim text-xs font-mono mt-1 opacity-60">
                    Used by Terraform Scanner when requesting IaC context.
                  </p>
                </div>
              </div>
            </section>
